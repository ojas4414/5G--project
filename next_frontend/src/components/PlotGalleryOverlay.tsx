...
type TabKey = 'core' | 'publication' | 'legacy';

const EMPTY_MANIFEST: PlotManifest = {
  core: [],
  publication: [],
  legacy: [],
  counts: { core: 0, publication: 0, legacy: 0 },
};
...
export function PlotGalleryOverlay({ open, onClose, refreshToken }: PlotGalleryOverlayProps) {
  ...
  const fetchPlots = async () => {
    ...
    try {
      ...
      setManifest(data);
      if (data.core.length === 0 && data.publication.length > 0) {
